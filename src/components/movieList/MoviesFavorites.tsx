...
const MoviesFavorites: React.FC = () => {
  ...
  function deleteFavorite(movieId: number) {

    MovieService.deleteUserFavorites(user.id_users!, movieId)
      .then(res => {
        if(res !== undefined) {
          ...
          {showedMovies.map((movie, key) => {
            return(
              <div key={key} className="movieCard">
                <i className="fas fa-times" onClick={() => deleteFavorite(movie.id_movies!)}></i>
